...
```sh
# モニターインターフェースをターゲットのチャネルに固定
sudo iw dev wlan0mon set channel 11
```

```sh
# 辞書攻撃
sudo mdk4 wlan0mon p -t F0:9F:C2:6A:88:26 -f /home/user/wordlists/ssid-names.txt
...
ビーコンを待つのではなく、能動的にネットワーク名を指定してネットワークを検索する必要がある。
