...
### ゲストの画面サイズがウィンドウサイズに追随しない

VMタイプをDebianからUbuntuに変えたら追随するようになった。（KaliはUbuntuではないが）

### マウスカーソルが表示されない

1. 仮想マシンをシャットダウンする:必須。Kali Linuxの画面から、OSをシャットダウンして完全に電源が切れた状態にする。
2. ハードウェア互換性の変更を開く:VMware側の操作。VMwareのライブラリ（左側のリスト）で対象の Kali Linux仮想マシンを右クリック ＞ 「管理」 ＞ 「ハードウェア互換性の変更」 を選択。
3. 最新のハードウェア互換性を選択する:バージョン指定。ウィザードが開くので「次へ」進み、「ハードウェア互換性」のプルダウンリストから、使っている環境における**最新のバージョン（例: Workstation 17.x またはそれ以上）**を選択して完了。
4. 起動して確認:完了。仮想マシンの電源を入れ、ログイン画面やデスクトップでマウスカーソルが復活しているか確認
...
### [pyinstxtractor_ng](https://github.com/pyinstxtractor/pyinstxtractor-ng)
...
python ./pyinstxtractor-ng/pyinstxtractor_ng.py ./program
```
