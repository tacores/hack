...
- もともとModbusはRS-232またはRS-485ケーブルを用いたシリアル接続で動作していたため配線への物理的なアクセスが必要だったが、現代の産業システムでは、Modbusプロトコルを標準のTCP /IPネットワークパケット内にカプセル化するModbus TCP（デフォルトで502ポート）が使用されている。
- Modbus にはセキュリティメカニズムが組み込まれていない。
- Modbusアドレスは0始まり

**デフォルトは502ポート。5020ポートもModbusの可能性高い。**

### レジスタタイプ

- 離散入力（デジタル入力）：1ビット(0/1)、RO
- コイル（デジタル出力）：1ビット(0/1)、R/W
- 入力レジスタ（アナログ入力）: 16ビット(0-65535)、RO
- 保持レジスタ（アナログ出力）: 16ビット(0-65535) R/W
