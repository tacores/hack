...
| 4625        | ログオン失敗 |   |
| 4634        | ログオフ |   |
| 4648        | 明示的な資格情報を利用したログオン | プロセスが現在ログオンしているユーザーの資格情報以外の資格情報を使用した場合に、ソースマシン上で発生 |
| 5140        | SMB共有アクセス |   |

| ログオンタイプ | 意味                         | 共通プロトコル | 説明                         |
|----------------|------------------------------|----------------|--------------------------------------------------------|
| 3              | ネットワークログオン         | SMB、PsExec    | 対話型セッションなしのリモートアクセス                 |
| 7              | ロック解除/再接続            | RDP            | セッションの再接続またはワークステーションのロック解除 |
| 10             | リモートインタラクティブ     | RDP            | フルデスクトップセッション                             |

※セッションがアクティブまたは切断中にRDP接続された場合、タイプは7になることに注意。（3,10だけだと考えていると見逃す）
...
4732はローカルグループへのメンバー追加。

- 4728: グローバル セキュリティ グループにメンバーが追加された
- 4756: ユニバーサル セキュリティ グループにメンバーが追加された
- 4732: ドメイン ローカル セキュリティ グループにメンバーが追加された（確認済み）
- 4738: ユーザー アカウントが変更された（UserAccountControl や属性変更）
