...
このページで、vol というコマンドが使われているときは Volatility3、volatility,vol.py というコマンドが使われているときは volatility2 を意味している。
...
このように階層を深堀していって値にたどり着くことができる。なお、vol2 には shellbags プラグインがある。
...
```sh
# パスと inode番号 が表示される
vol.py -f ./linux.mem --profile Linux5_4_0x64 linux_enumerate_files > enumerate_files
```

```sh
# ファイル一覧を取得（inode番号とアドレスの違いに注意）
vol.py -f ./linux.mem --profile=Linux5_4_0x64 linux_find_file -L > find_file_list

# 特定のinodeアドレスからファイルを取り出す
vol.py -f ./linux.mem --profile=Linux5_4_0x64 linux_find_file -i 0xINODE_ADDRESS -O output_file
```

## Linuxプロファイルの作成

https://github.com/Abyss-W4tcher/volatility2-profiles/tree/master で探せる可能性はある。

### vol2

#### 環境特定

vol3 でバナーを出力。`Ubuntu20.04.2, 5.4.0-166-generic カーネル`のメモリダンプと判明。

```sh
$ vol -f ./linux.mem banners                                 
Volatility 3 Framework 2.27.0
Progress:  100.00               PDB scanning finished                  
Offset  Banner
...
```sh
$ sudo docker run --rm -v $(pwd):/output \
  ubuntu:20.04 bash -c "
    apt-get update && \
    apt-get install -y build-essential dwarfdump git zip \
      linux-headers-5.4.0-166-generic \
      linux-image-5.4.0-166-generic && \
    git clone https://github.com/volatilityfoundation/volatility.git && \
    cd volatility/tools/linux && \
    make KVER=5.4.0-166-generic KDIR=/ && \
    cp /boot/System.map-5.4.0-166-generic . && \
    zip 5.4.0.zip module.dwarf System.map-5.4.0-166-generic && \
    cp 5.4.0.zip /output/
"
...
```sh
cp ./5.4.0.zip ~/.local/lib/python2.7/site-packages/volatility/plugins/overlays/linux
```
...
https://www.iblue.team/memory-forensics-1/volatility-plugins/build-custom-linux-profile-for-volatility

#### コンテナでプラグインをMake

dwarf2json を使うのが vol2 との相違点。
...
$ sudo docker run --rm -v $(pwd):/output \
  ubuntu:20.04 bash -c "
    ...
    echo 'deb http://ddebs.ubuntu.com focal main restricted universe multiverse' > /etc/apt/sources.list.d/ddebs.list && \
    echo 'deb http://ddebs.ubuntu.com focal-updates main restricted universe multiverse' >> /etc/apt/sources.list.d/ddebs.list && \
    echo 'deb http://ddebs.ubuntu.com focal-proposed main restricted universe multiverse' >> /etc/apt/sources.list.d/ddebs.list && \
    apt-get update && \
    apt-get install -y \
      linux-image-5.4.0-166-generic-dbgsym \
      linux-image-5.4.0-166-generic && \
    ...
    ./dwarf2json-linux-amd64 linux \
      --elf /usr/lib/debug/boot/vmlinux-5.4.0-166-generic \
      --system-map /boot/System.map-5.4.0-166-generic \
      > /output/linux-5.4.0-166-generic.json
"

$ ls
linux-5.4.0-166-generic.json
```

#### 環境にコピー

symbols直下にコピーする。linuxディレクトリを作って入れても認識しなかった。

```sh
cp ./linux-5.4.0-166-generic.json ~/myenv/lib/python3.13/site-packages/volatility3/framework/symbols
```
...
file:///home/kali/myenv/lib/python3.13/site-packages/volatility3/framework/symbols/linux-5.4.0-166-generic.json Unknown   18      10294   162829  1737    b'Linux version 5.4.0-166-generic (buildd@lcy02-amd64-011) (gcc version 9.4.0 (Ubuntu 9.4.0-1ubuntu1~20.04.2)) #183-Ubuntu SMP Mon Oct 2 11:28:33 UTC 2023 (Ubuntu 5.4.0-166.183-generic 5.4.252)\n\x00'
```
