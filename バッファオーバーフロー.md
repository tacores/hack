...
- canaryの先頭バイト（LSB）は必ず00
- ランダム配置の場合、関数アドレスの最後の3桁は不変

### libcバージョン

puts等のGOTを取得し、https://libc.rip/ でバージョンを絞る。
...
ランダムアドレスでも下3桁は変わらない点に注目して、どの関数アドレスが露出しているか推測することができる。
...
| ガジェット                 | 用途                 |
| --------------------- | ------------------ |
| `pop rdi; ret`        | 関数の第1引数をセット        |
| `pop rsi; ret`        | 第2引数をセット           |
| `pop rdx; ret`        | 第3引数をセット           |
| `pop rcx; ret`        | 第4引数（Windowsなどで使用） |
