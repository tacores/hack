...
```sh
# 16文字の文字列をHEXで渡している。下記の場合はNULL文字でパディング。
...
# 4. 共通鍵をそれぞれ計算
# アリス: K_A = B^a mod p = (g^b)^a mod p = g^(ab) mod p
K_A = pow(B, a, p)

# ボブ:   K_B = A^b mod p = (g^a)^b mod p = g^(ab) mod p
K_B = pow(A, b, p)
```
