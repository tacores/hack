...
- IDベースポリシー：プリンシパル（ユーザー、グループ、またはロール）がリソースに対して実行できるアクションを定義する。
- リソースベースポリシー：誰がこのリソース（リソース、グループ、ロール）にアクセスできるのか、そしてどのようにアクセスできるのか？を定義する。
- ロールには信頼ポリシーとIDベースポリシーの両方が添付されている必要がある。
...
Principal に root を設定すると、rootユーザーだけではなく、このアカウント内のすべてのプリンシパルが信頼されるという意味になってしまう。

https://tryhackme.com/room/bypassingcloudsecurity

```json
{
    "Sid": "OverPermissiveTrust",
    "Effect": "Allow",
    "Principal": {
