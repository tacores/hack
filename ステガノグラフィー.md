...
### [stegpy](https://github.com/izcoser/stegpy)

numpyの大幅仕様変更により簡単に復元できなくなっていたりと安定性がない。
...
AppImageファイルをダウンロードして、実行ファイルとして実行する。  
...
# 古すぎて動かないので調整
sed -i 's/FROM debian:stretch-20170907/FROM debian:bookworm/' ./stego-toolkit/Dockerfile
sed -i '/python-pip/d' ./stego-toolkit/Dockerfile
sed -i 's/pip3 install/pip3 install --break-system-packages/g' ./stego-toolkit/Dockerfile

# 10分以上かかる
sudo ./stego-toolkit/bin/build.sh

# コンテナのルートシェルが表示される
sudo ./stego-toolkit/bin/run.sh
```
