...
0xff を符号付として扱う場合、0x00000000000000ff ではなく 0xffffffffffffffff としてコピーされなければならない。
...
オペランドには必ず角括弧が付く。メモリは参照せず、アドレスを計算するだけ。  
...
rdx : rax by rcx -> rax, rdx （raxが商、rdxが余り）

div 10 の後、raxは十の位を保持し、rdxは一の位を保持する。
...
cmp でメモリ逆参照ができるのは、最大１つのオペランドのみ。  
また、即値は第2オペランド（左側）に書かないとエラーになる。
...
rdi の 下位8ビットに ZF の値を書き込む。setzで書き込めるのは8ビットレジスタのみ。
...
- `rax, rcx, rdx, rsi, rdi, r8, r9, r10, r11` は、callee によって自由に破壊される可能性があるので、caller は（必要なら）自分で保存、復元する責任がある。
- `rbx, rbp, r12, r13, r14, r15` は、callee が勝手に壊してはならない。callee が使う場合、もとに復元してから ret する責任がある。
...
argv の後に、環境変数 envp がある。`NAME=VALUE` の形の文字列でスタック上に格納される。
...
gdb で起動するとき、gdbが独自の環境変数を挿入するため、普通に起動した場合のスタックアドレスとずれが生じる。下記のようにパディングで調整することができる。

```sh
env -i FOO=aaaaaaa /program
```
...
_start:
    ...
    lea rdi, [rip+path] // mov ではアドレスではなく値になる。
                        // +path は現在地点からpathまでのアドレスの差になる。
